...
static std::unique_ptr<SpectralUpsampler> sSpectralUpsampler;
...
static rgb upsample_rgb(const rgb &c)
{
    rgb v;
    sSpectralUpsampler->prepare(&c.x, &c.y, &c.z, &v.x, &v.y, &v.z, 1);
    return v;
}
...
static size_t cleanup_obj(obj::File &obj_file, obj::MaterialLib &mtl_lib)
{
    ...
    dummy_mat.ka = upsample_rgb(rgb(0.0f));
    dummy_mat.kd = upsample_rgb(rgb(0.0f, 1.0f, 1.0f));
    dummy_mat.ks = upsample_rgb(rgb(0.0f));
    dummy_mat.ke = upsample_rgb(rgb(0.0f));
    dummy_mat.ns = 1.0f;
    dummy_mat.ni = 1.0f;
    dummy_mat.tf = upsample_rgb(rgb(0.0f));
    dummy_mat.tr = 1.0f;
    dummy_mat.d = 1.0f;
    dummy_mat.illum = 2;
    dummy_mat.map_ka = "";
    ...
    if (has_simple)
    {
        ...
        for (size_t i = 0, j = 0; i < tri_mesh.indices.size(); i += 4, j++)
        {
            ...
            if (geom_id >= num_complex)
            {
                ...
                simple_kd[j] = upsample_rgb(mat.kd);
                simple_ks[j] = upsample_rgb(mat.ks);
                simple_ns[j] = mat.ns;
                geom_id = num_complex;
            }
            else
            {
                simple_kd[j] = upsample_rgb(float3(0.1f, 0.05f, 0.01f));
                simple_ks[j] = upsample_rgb(float3(0.1f, 0.05f, 0.01f));
                simple_ns[j] = 1.0f;
            }
        }
...
int main(int argc, char **argv)
{
    ...
    try
    {
        sSpectralUpsampler = std::make_unique<SpectralUpsampler>("srgb.coeff");
    }
